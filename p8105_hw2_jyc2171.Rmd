...
nyc_transit =
  ...
  mutate(
    ...
    ada = as.numeric(pull(nyc_transit, ada)),
    route8 = as.character(pull(nyc_transit, route8)),
    route9 = as.character(pull(nyc_transit, route9)),
    route10 = as.character(pull(nyc_transit, route10)),
    route11 = as.character(pull(nyc_transit, route11)),
    entrance_type = str_to_lower(entrance_type)
  )

## filter vending == "0" and entry == "1" and provide column == station_name, nrows
...
## filter(nyc_transit, ada_number == 1) %>% 
## distinct(station_name) %>% 
## nrow()

## mutate(ada_numeric = as.numeric(pull(nyc_transit, ada)) %>% 
## relocate(ada_numeric)
...
The data has been reformatted to longer, generating a new dataframe that consists of `r nrow(transit_tidy)` rows and `r ncol(transit_tidy)` columns.  There are `r filter(transit_tidy, train_name == "A") %>% distinct(station_name)` distinct stations that serve the A train.
